...
import { appStateBus } from "./EventBus";

const DEFAULT_FEATURE_LAYER = "https://services1.arcgis.com/0MSEUqKaxRlEPj5g/arcgis/rest/services/Hospitals2/FeatureServer/0/query";

export function App(): React.JSX.Element {
  const [activeLayerUrl, setActiveLayerUrl] = useState<string>("");
  const [draftLayerUrl, setDraftLayerUrl] = useState<string>(DEFAULT_FEATURE_LAYER);
  ...
  React.useEffect(() => {
    const unsub = appStateBus.subscribe((state) => {
  ...
  const applyLayerUrl = (): void => {
    ...
    try {
      ...
      if (!parsed.protocol.startsWith("http")) {
        setLayerModalError("Use an http or https ArcGIS FeatureServer layer URL.");
        return;
      }
    } catch {
  ...
  return (
    <calcite-shell className="app-root">
      <calcite-navigation slot="header" style={{ height: "56px", borderBottom: '1px solid #e2e8f0', zIndex: 100, position: 'relative' }}>
        <Box slot="logo" sx={{ display: 'flex', alignItems: 'center', px: { xs: 1.5, sm: 3 }, gap: { xs: 1.5, sm: 2 }, height: '100%' }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <Typography variant="h6" sx={{ fontWeight: 800, color: '#111827', lineHeight: 1.1, letterSpacing: '-0.5px', fontSize: { xs: '1.1rem', sm: '1.25rem' } }}>
              Incident Mapper
            </Typography>
            <Typography variant="caption" sx={{ color: '#6b7280', fontWeight: 600, letterSpacing: '0.2px', fontSize: { xs: '0.65rem', sm: '0.75rem' } }}>
              Live Spatial Intelligence
            </Typography>
          </Box>
        </Box>
        <Box slot="content-end" sx={{ display: "flex", alignItems: "center", mr: { xs: 1.5, sm: 3 }, height: '100%' }}>
          <MapSettingsPanel settings={mapSettings} onSettingsChange={setMapSettings} />
          <Button 
            variant="contained" 
            disableElevation 
        ...
        {isMobile ? (
          <>
            <Box sx={{ height: "100%", width: "100%" }}>
              <MapPanel layerUrl={activeLayerUrl} settings={mapSettings} filterClause={filterClause} />
            </Box>
            <BottomSheet>
              <ListPanel layerUrl={activeLayerUrl} settings={mapSettings} filterClause={filterClause} onFilterChange={setFilterClause} />
            </BottomSheet>
          </>
        ) : (
          <>
